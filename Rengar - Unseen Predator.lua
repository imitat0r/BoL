...
function AfterDownload()
	...
	if DOWNLOAD_COUNT == 0 then
		...
		print("<b><font color=\"#6699FF\">Rengar - Unseen Predator:</font></b> <font color=\"#FFFFFF\">Required libraries downloaded successfully, please reload (double F9).</font>")
	end
end
...
local UPDATE_NAME = "Rengar - Unseen Predator"
local UPDATE_HOST = "raw.github.com"
local UPDATE_PATH = "/DraconisBoL/BoL/master/Rengar%20-%20Unseen%20Predator.lua" .. "?rand=" .. math.random(1, 10000)
...
function OnLoad()
	print("<b><font color=\"#6699FF\">Rengar - Unseen Predator:</font></b> <font color=\"#FFFFFF\">Good luck and have fun!</font>")
	print("<b><font color=\"#6699FF\">Rengar - Unseen Predator:</font></b> <font color=\"#FFFFFF\">Team #SWAGelo: Draconis, Fantastik and Jaikor!</font>")
	Variables()
	Menu()
	PriorityOnLoad()
end
...
function OnProcessSpell(unit, spell)
	if unit.isMe and spell.name == "RengarR" then
		ThrillOfTheHunt = true
	end
	if unit.isMe and spell.name == ("BasicAttack" or "CritAttack") and ThrillOfTheHunt == true then
		ThrillOfTheHunt = false
	end
end
...
function Checks()
	...
	if VIP_USER and _G.SkinHack and Settings.misc.skinList then ChooseSkin() end
...
function Variables()
	SkillQ = { name = "Savagery", range = TrueRange(), delay = nil, speed = nil, width = nil, ready = false }
	SkillW = { name = "Battle Roar", range = 400, delay = nil, speed = nil, width = nil, ready = false }
...
function ChooseSkin()
	if Settings.misc.skinList ~= lastSkin then
		...
		GenModelPacket("Rengar", Settings.misc.skinList)
	end
end
...
-- Deaddevil2
function Magnet()
    if Target ~= nil and (ComboKey or HarassKey) and ValidTarget(Target, (TrueRange() + 50)) then
...
function StayClose(unit, mode)
	if mode then
		...
		if GetDistance(ClosePoint1) < GetDistance(ClosePoint2) then
			SOWi:OrbWalk(unit, ClosePoint1)
		else
			SOWi:OrbWalk(unit, ClosePoint2)
		end
	else
		SOWi:OrbWalk(unit, myHero)
	end
end
