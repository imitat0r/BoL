...
function AfterDownload()
	...
	if DOWNLOAD_COUNT == 0 then
		...
		print("<b><font color=\"#6699FF\">Shyvana - Flame Breath:</font></b> <font color=\"#FFFFFF\">Required libraries downloaded successfully, please reload (double F9).</font>")
	end
end

for DOWNLOAD_LIB_NAME, DOWNLOAD_LIB_URL in pairs(REQUIRED_LIBS) do
	if FileExist(LIB_PATH .. DOWNLOAD_LIB_NAME .. ".lua") then
		if DOWNLOAD_LIB_NAME then require(DOWNLOAD_LIB_NAME) end
	else
	...
	for _, enemy in ipairs(GetEnemyHeroes()) do
		...
		if ValidTarget(enemy) and enemy.visible then
			if enemy.health <= qDmg and GetDistance(enemy) <= SkillQ.range then
				CastQ(enemy)
			elseif enemy.health <= (qDmg + eDmg) and (GetDistance(enemy) <= SkillQ.range and GetDistance(enemy) <= SkillE.range)  then
				CastE(enemy)
				CastQ(enemy)
...
function Menu()
	Settings = scriptConfig("Shyvana - Flame Breath "..version.."", "DraconisShyv")
...
function Variables()
	SkillQ = { name = "Twin Bite", range = TrueRange(), delay = nil, speed = nil, width = nil, ready = false }
...
function PriorityOnLoad()
	if heroManager.iCount < 10 or (TwistedTreeline and heroManager.iCount < 6) then
		print("<b><font color=\"#6699FF\">Shyvana - Flame Breath:</font></b> <font color=\"#FFFFFF\">Too few champions to arrange priority.</font>")
...
function ChooseSkin()
	if Settings.misc.skinList ~= lastSkin then
		...
		GenModelPacket("Shyvana", Settings.misc.skinList)
	end
end
