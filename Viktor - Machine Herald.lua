...
id = 44
ScriptName = "ViktorMachineHerald"
...
function AfterDownload()
	...
	if DOWNLOAD_COUNT == 0 then
		...
		print("<b><font color=\"#6699FF\">Viktor - Machine Herald:</font></b> <font color=\"#FFFFFF\">Required libraries downloaded successfully, please reload (double F9).</font>")
	end
end
...
local UPDATE_NAME = "Viktor - Machine Herald"
local UPDATE_HOST = "raw.github.com"
local UPDATE_PATH = "/DraconisBoL/BoL/master/Viktor%20-%20Machine%20Herald.lua" .. "?rand=" .. math.random(1, 10000)
...
function OnLoad()
	print("<b><font color=\"#6699FF\">Viktor - Machine Herald:</font></b> <font color=\"#FFFFFF\">Sucessfully loaded!</font>")
	...
	if JungleClearKey then
		...
		if JungleMob ~= nil then
			...
			if Settings.jungle.jungleE and GetDistance(JungleMob) <= SkillE.range and SkillE.ready then
				if VIP_USER and Settings.misc.packets then Packet("S_CAST", { spellId = _R, toX = JungleMob.x, toY = JungleMob.z, fromX = JungleMob.x, fromY = JungleMob.z }):send() end
...
function CastR(unit)
	if unit ~= nil and GetDistance(unit) <= SkillR.range and SkillR.ready then
		local AOECastPosition, MainTargetHitChance, nTargets = VP:GetCircularAOECastPosition(unit, SkillR.delay, SkillR.width, SkillR.range, SkillR.speed, myHero)
...
function Menu()
	Settings = scriptConfig("Viktor - Machine Herald "..version.."", "DraconisViktor")
	...
	Settings:addSubMenu("["..myHero.charName.."] - Lane Clear Settings", "lane")
		...
		Settings.lane:addParam("laneE", "Clear with "..SkillR.name.." (E)", SCRIPT_PARAM_ONOFF, true)
	...
	Settings:addSubMenu("["..myHero.charName.."] - Misc Settings", "misc")
		...
		Settings.misc:addParam("skinList", "Choose your skin", SCRIPT_PARAM_LIST, 4, { "Full Machine Viktor", "Prototype Viktor", "Creator Viktor", "Classic" })
...
function Variables()
	SkillQ = { name = "Power Transfer", range = 600, delay = nil, speed = nil, width = nil, ready = false }
...
function PriorityOnLoad()
	if heroManager.iCount < 10 or (TwistedTreeline and heroManager.iCount < 6) then
		print("<b><font color=\"#6699FF\">Viktor - Machine Herald:</font></b> <font color=\"#FFFFFF\">Too few champions to arrange priority.</font>")
...
function ChooseSkin()
	if Settings.misc.skinList ~= lastSkin then
		...
		GenModelPacket("Viktor", Settings.misc.skinList)
	end
end
