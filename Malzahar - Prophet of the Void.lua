...
function OnLoad()
	print("<b><font color=\"#6699FF\">Malzahar - Prophet of the Void:</font></b> <font color=\"#FFFFFF\">Good luck and have fun!</font>")
	Variables()
	Menu()
	PriorityOnLoad()
end
...
function OnAnimation(unit, animationName)
	if unit.isMe then
		if animationName == "Spell4" or animationName == "spell4" or animationName == "Spell4_Windup" or animationName == "Spell4_Winddown" or animationName == "Spell4_Loop" or animationName == "AlZaharNetherGrasp" then
			NetherGrasp = true
		else
			NetherGrasp = false
		end
...
function Combo(unit)
	if ValidTarget(unit) and unit ~= nil and unit.type == myHero.type then	
		...
		CastW(unit)
		CastE(unit)
		if Settings.combo.comboItems then return end
...
function CastW(unit)
	if Settings.combo.useW == 1 or NetherGrasp == true then return end
...
function Menu()
	Settings = scriptConfig("Malzahar - Prophet of the Void "..version.."", "DraconisMalza")
	...
	TargetSelector.name = "Malzahar"
	Settings:addTS(TargetSelector)
end

function Variables()
	SkillQ = { name = "Call of the Void", range = 900, delay = 0.6, speed = 1600, width = 450, ready = false }
	SkillW = { name = "Null Zone", range = 800, delay = 0.25, speed = math.huge, width = 250, ready = false }
	SkillE = { name = "Malefic Visions", range = 650, delay = nil, speed = nil, width = nil, ready = false }
	SkillR = { name = "Nether Grasp", range = 700, delay = nil, speed = nil, width = nil, ready = false }
...
function PriorityOnLoad()
	if heroManager.iCount < 10 or (TwistedTreeline and heroManager.iCount < 6) then
		print("<b><font color=\"#6699FF\">Malzahar - Prophet of the Void:</font></b> <font color=\"#FFFFFF\">Too few champions to arrange priority.</font>")
