...
function AfterDownload()
	...
	if DOWNLOAD_COUNT == 0 then
		...
		print("<b><font color=\"#6699FF\">Maokai - Der gewundene Treant:</font></b> <font color=\"#FFFFFF\">Required libraries downloaded successfully, please reload (double F9).</font>")
	end
end
...
local UPDATE_NAME = "Maokai - Der gewundene Treant"
local UPDATE_HOST = "raw.github.com"
local UPDATE_PATH = "/DraconisBoL/BoL/master/Maokai%20-%20Der%20gewundene%20Treant.lua" .. "?rand=" .. math.random(1, 10000)
...
function OnLoad()
	print("<b><font color=\"#6699FF\">Maokai - Der gewundene Treant:</font></b> <font color=\"#FFFFFF\">Good luck and have fun!</font>")
	Variables()
	Menu()
	PriorityOnLoad()
end
...
function CastE(unit)
	if unit ~= nil and GetDistance(unit) <= SkillE.range and SkillE.ready then
		local AOECastPosition, MainTargetHitChance, nTargets = VP:GetLineCastPosition(unit, SkillE.delay, SkillE.width, SkillE.range, SkillE.speed, myHero)
...
function CastR(unit)
	if Settings.combo.useR == 1 or VengefulMaelstrom then return end
...
function Menu()
	Settings = scriptConfig("Maokai - Der gewundene Treant "..version.."", "DraconisMaokai")
	...
	TargetSelector = TargetSelector(TARGET_LESS_CAST, SkillE.range, DAMAGE_MAGICAL, true)
	TargetSelector.name = "Maokai"
	Settings:addTS(TargetSelector)
end

function Variables()
	SkillQ = { name = "Arcane Smash", range = 600, delay = 0.5, speed = 1200, width = 110, ready = false }
	SkillW = { name = "Twisted Advance", range = 525, delay = nil, speed = nil, width = nil, ready = false }
	SkillE = { name = "Sapling Toss", range = 1100, delay = 1.0, speed = 1500, width = 250, ready = false }
	SkillR = { name = "Vengeful Maelstrom", range = 475, delay = nil, speed = nil, width = nil, ready = false }
...
function PriorityOnLoad()
	if heroManager.iCount < 10 or (TwistedTreeline and heroManager.iCount < 6) then
		print("<b><font color=\"#6699FF\">Maokai - Der gewundene Treant:</font></b> <font color=\"#FFFFFF\">Too few champions to arrange priority.</font>")
