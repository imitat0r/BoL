...
function AfterDownload()
	...
	if DOWNLOAD_COUNT == 0 then
		...
		print("<b><font color=\"#6699FF\">Cassiopeia - Deadly Cadence:</font></b> <font color=\"#FFFFFF\">Required libraries downloaded successfully, please reload (double F9).</font>")
	end
end
...
local UPDATE_NAME = "Cassiopeia - Deadly Cadence"
local UPDATE_HOST = "raw.github.com"
local UPDATE_PATH = "/DraconisBoL/BoL/master/Cassiopeia%20-%20Deadly%20Cadence.lua" .. "?rand=" .. math.random(1, 10000)
...
function OnLoad()
	print("<b><font color=\"#6699FF\">Cassiopeia - Deadly Cadence:</font></b> <font color=\"#FFFFFF\">Good luck and have fun!</font>")
	Variables()
	Menu()
	PriorityOnLoad()
end
...
function OnTick()
	...
	if FarmKey and Settings.farm.farmE then
		CastAutoE()
	end
	...
	if FarmKey and not IsMyManaLow("Farm") then
		for i, minion in pairs(enemyMinions.objects) do
			if ValidTarget(minion) and minion ~= nil then
				...
				if GetDistance(minion) <= SkillE.range and minion.health <= eDmg and SkillE.ready then
					if isPoisoned(minion) then
						CastSpell(_E, minion)
					end
				end
	...
	for _, enemy in ipairs(GetEnemyHeroes()) do
		if ValidTarget(enemy) and enemy.visible then
			...
			if enemy.health <= qDmg and GetDistance(enemy) <= SkillQ.range then
				CastQ(enemy)
			elseif enemy.health <= eDmg and GetDistance(enemy) <= SkillE.range then
				CastE(enemy)
			elseif Settings.ks.kSR and enemy.health <= rDmg and GetDistance(enemy) <= SkillR.range and SkillR.ready then
...
function Menu()
	...
	Settings:addSubMenu("["..myHero.charName.."] - Farm Settings", "farm")
		...
		Settings.farm:addParam("farmE", "Farm with "..SkillE.name.." (E)", SCRIPT_PARAM_ONOFF, true)
	...
	TargetSelector.name = "Cassiopeia"
...
function PriorityOnLoad()
	if heroManager.iCount < 10 or (TwistedTreeline and heroManager.iCount < 6) then
		print("<b><font color=\"#6699FF\">Cassiopeia - Deadly Cadence:</font></b> <font color=\"#FFFFFF\">Too few champions to arrange priority.</font>")
...
function isPoisoned(target)
	...
	local tBuff = target:getBuff(i)
		if BuffIsValid(tBuff) then
			if tBuff.name:lower():find("poison") and (tBuff.endT - (math.max(GetDistance(target), 700)/1800 + 0.200) - GetGameTimer() > 0) then
				return true
			end
		end
	end
